	...
	if(isset($_POST['notificationType'])){
		if ($_POST['notificationType'] == 'transaction'){
			...
			try{
				...
				var_dump('>>>>>' . $configucao_ecommerce->is_send_order_email);
				if ($configucao_ecommerce->is_send_order_email){
					/* ***********************************
						ENVIA PEDIDO POR E-MAIL
					*********************************** */

					$_pedidoemail_entidade_id 	= getEntidadeId('td_ecommerce_pedidoemail');
					$_email_entidade_id			= getEntidadeId('td_email');
					$sqlPedidoEmail = "
						SELECT * FROM td_lista 
						WHERE entidadepai = {$_pedidoemail_entidade_id}
						and entidadefilho = {$_email_entidade_id}
						AND regpai = 1;
					";
					echo $sqlPedidoEmail;
					...
					if ($queryPedidoEmail->rowCount() > 0){
						...
						$emailRemetente		= getRegistro($conn,"email","*","id=".$linhaPedidoEmail["regpai"],"LIMIT 1");
						...
						$mail->IsHTML(true);					
						$msg = file_get_contents(URL_MILES. "index.php?controller=website/ecommerce/pedidoenvioemail/pedidoenvioemail&registro=1&currentproject=27&key=k");
						$mail->Body = $msg;
						if(!$mail->Send())
						{
							echo $mail->ErrorInfo;
