	...
	if ($_POST){
		if ($_POST["op"] == "salvar"){
			...
			if ($conn = Transacao::get()){
				try{
					$codigo = $conn->query("SELECT IFNULL(MAX(codigo),0)+1 FROM td_relacaocredores WHERE td_processo = 3");
					$cod = $codigo->fetch();
					$credor = tdClass::Criar("persistent",array("td_relacaocredores"));
					$credor->contexto->codigo = $cod[0];
					$credor->contexto->td_tipo = 1;
					$credor->contexto->td_origemcredor = 1;
					$credor->contexto->nome = $nome;
					$credor->contexto->cnpj = $cnpj;
					$credor->contexto->td_moeda = 1;
					$credor->contexto->valor = $valor;
					$credor->contexto->td_processo = 3;
					...
					$credor->contexto->td_tipoempresa = $tipoempresa;
