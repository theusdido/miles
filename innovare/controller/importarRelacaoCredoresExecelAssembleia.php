	...
	*/
	...
	if (isset($_GET["op"])){
		if ($_GET["op"] == "excluircredores"){
			$conn = Transacao::Get();
			$sql = "SELECT id FROM td_relacaocredores WHERE td_tipo = 0;";
			$query = $conn->query($sql);
			while ($linha = $query->fetch()){
				$conn->exec("DELETE FROM td_relacaocredores WHERE id = " . $linha["id"]);
				$conn->exec("DELETE FROM td_lista WHERE entidade_pai = 16 AND entidade_filho = 20 AND reg_pai = 18 AND reg_filho = " . $linha["id"]);
			}
			Transacao::Fechar();
		}
	}
	if (isset($_FILES["arquivo"])){
		...
		if ($conn = Transacao::get()){
			...
			if (isset($_POST["origem_credor_4"])) array_push($_origem_credor,'td_origemcredor = 4');
			if (isset($_POST["origem_credor_7"])) array_push($_origem_credor,'td_origemcredor = 7');
			if (isset($_POST["origem_credor_8"])) array_push($_origem_credor,'td_origemcredor = 8');
			...
			foreach ($xml->Worksheet->Table->Row as $cell){
				...
				$sqlExiste = "
					...
					AND td_classificacao = {$classificacao} 
					AND td_processo = {$processo[1]} 
