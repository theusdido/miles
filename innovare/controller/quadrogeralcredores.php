<?php
	...
	if (isset($_POST["op"])){
		...
		if ($conn = Transacao::get()){
			...
			$conn->exec("DELETE FROM td_relacaocredores WHERE td_processo = $processo AND farein = $farein AND td_origemcredor = 7");
			...
			while ($linha_credor = $query_credor->fetch()){
				...
				if ($linha_credor["td_origemcredor"] == 5 || $linha_credor["td_origemcredor"] == 6){
					$sql_imphab = "	SELECT id,parecervalor,td_parecermoeda,td_parecerclassificacao FROM td_habilitacaoimpugnacao WHERE td_credor = {$linha_credor["id"]} AND decisao = 1";
					$query_imphab = $conn->query($sql_imphab);
					while ($linha_imphab = $query_imphab->fetch()){
						
						$sqlParecer = "SELECT legitimidade,valor,td_moeda,td_classificacao,cpfj FROM td_habilitacaoimpugnacaoparecer WHERE td_habilitacaoimpugnacao = {$linha_imphab["id"]} AND valor > 0";
				...
				}else if($linha_credor["td_origemcredor"] == 4){
					$valor			= $linha_credor["valor"];	
					$moeda			= $linha_credor["td_moeda"];
					$classificacao	= $linha_credor["td_classificacao"];
	...
	if (isset($_GET["idpk"])){
		...
		foreach($dsHabDiv as $habdiv){
			if ($habdiv->td_credor != null && $habdiv->td_credor != "" && is_numeric($habdiv->td_credor)){
				...
				if ($credor->contexto->farein == $pk_id[1] && ($credor->contexto->td_origemcredor == 5 || $credor->contexto->td_origemcredor == 6)){
		...
		$sqlCredores->addFiltro("td_origemcredor","=",4);
