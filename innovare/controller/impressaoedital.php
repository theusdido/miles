	...
	<body>
		<?php
			...
			if ($conn = Transacao::get()){		
				$sql = "SELECT td_comarca,td_juizo,td_cidade,logradouro,bairro,cep,telefone,td_estado,email,td_magistrado,escrivao,numeroprocesso,dipositivodecisao,resumopedidorecuperacao,tipoprocesso,complemento FROM td_processo WHERE id = $processo;";
				$query = $conn->query($sql);
				if ($linha = $query->fetch()){
					$comarca 		= tdClass::Criar("persistent",array("td_comarca",$linha["td_comarca"]));
					$vara			= tdClass::Criar("persistent",array("td_juizo",$linha["td_juizo"]));
					$cidade			= tdClass::Criar("persistent",array("td_cidade",$linha["td_cidade"]));
					$estado			= tdClass::Criar("persistent",array("td_estado",$linha["td_estado"]));
					...
					$magistrado		= tdClass::Criar("persistent",array("td_magistrado",$linha["td_magistrado"]));
					...
					if ($tipo_farein == "RE"){ // Recupera&ccedil;&atilde;o
						...
						$fareinID = 16;
						// Carrega Lista
						$sql = "SELECT reg_filho FROM ".LISTA." WHERE entidade_pai = 15 and entidade_filho = 16 and reg_pai = {$processo} and reg_filho = {$farein};";
						$query = $conn->query($sql);
						...
						$sql = "SELECT razaosocial FROM td_recuperanda WHERE id = " . $reg_fiho_recuperanda["reg_filho"];
						$query = $conn->query($sql);
						...
						$fareinID = 19;
						// Carrega Lista
						$sql = "SELECT reg_filho FROM ".LISTA." WHERE entidade_pai = 15 and entidade_filho = 19 and reg_pai = {$processo} and reg_filho = {$farein}";
						$query = $conn->query($sql);
						$reg_fiho_falida = $query->fetch();
						
						$sql = "SELECT razaosocial FROM td_falencia WHERE id = " . $reg_fiho_falida["reg_filho"];
						$query = $conn->query($sql);
						if ($linha = $query->fetch()){
							$razaosocial =  $linha["razaosocial"];
				...
				$sqlListaCredores = "SELECT reg_filho FROM td_lista c WHERE c.entidade_pai = {$fareinID} and c.entidade_filho = 20 and c.reg_pai = {$farein};";				
				$queryListaCredores = $conn->query($sqlListaCredores);
				$idsListaCredores = "";
				while ($linhaListaCredores = $queryListaCredores->fetch()){
					$idsListaCredores .= ($idsListaCredores==""?"":",") . $linhaListaCredores["reg_filho"];
				}
				...
				$sql = "SELECT a.id,a.descricao FROM td_classificacao a WHERE exists(SELECT 1 FROM td_relacaocredores b WHERE b.id in ({$idsListaCredores}) AND  b.td_classificacao = a.id AND b.farein = {$farein} AND b.td_processo = {$processo} AND (b.td_origemcredor is null OR b.td_origemcredor = 1)) ORDER BY a.ordem ASC ";;
				//echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>$sql<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<";
				$query = $conn->query($sql);
				$tr = "";
				$total = 0;
				...
				while ($linha = $query->fetch()){
					...
					$sql_credores = "SELECT nome,cpf,cnpj,valor,td_tipo,td_moeda FROM td_relacaocredores WHERE farein = {$farein} AND td_processo = {$processo} AND td_classificacao = {$linha["id"]} AND (td_origemcredor is null OR td_origemcredor = 1) ORDER BY nome";

					$query_credores = $conn->query($sql_credores);
					$moeda_atual = $valor = 0;
					$moeda_ultima = "";
					while ($linha_credores = $query_credores->fetch()){
						$moeda = tdClass::Criar("persistent",array("td_moeda",$linha_credores["td_moeda"]));
						$simboloMoeda = "";
						switch($linha_credores["td_moeda"]){
							case 1: $simboloMoeda = "R$"; break;
							case 2: $simboloMoeda = "US$"; break;
							case 3: $simboloMoeda = "EUR€"; break;
								
						}
						if ($linha_credores["td_tipo"] == '' || $linha_credores["td_tipo"] == 0){
							$tipocredor = '';
						}else{
							$tipocredor = ($linha_credores["td_tipo"]==1?$linha_credores["cnpj"]:$linha_credores["cpf"]) . " - ";
						}
						echo " " . strtoupper($linha_credores["nome"] . " - " . $tipocredor . ($simboloMoeda).moneyToFloat($linha_credores["valor"],true) . ";");
						if ($linha_credores["td_moeda"] == 1){
							$valorReal += $linha_credores["valor"];
						}
						if ($linha_credores["td_moeda"] == 2){
							$valorDollar += $linha_credores["valor"];
						}
						if ($linha_credores["td_moeda"] == 3){
							$valorEuro += $linha_credores["valor"];
						}				
						#$moeda_atual = $linha_credores["td_moeda"];
