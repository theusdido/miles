<?php
	...
	*/
	...
	if (isset($_FILES["arquivo"])){
		...
		if ($conn = Transacao::get()){			
			...
			foreach ($xml->Worksheet->Table->Row as $cell){
				...
				$sqlExiste = "SELECT id FROM td_relacaocredores WHERE (replace(replace(replace(cnpj,'/',''),'-',''),'.','') = '$cpfj' or replace(replace(cpf,'-',''),'.','') = '$cpfj') AND (cpf <> '$cpfj' or cnpj <> '$cpfj') AND td_classificacao = {$classificacao} AND td_processo = {$processo[1]} AND farein = {$processo[0]} AND td_moeda = {$moeda}";
				...
				if ($queryExiste->rowcount() > 0 && $cpfj != ""){
					...
					$credor->contexto->td_classificacao = $classificacao;
					$credor->contexto->td_natureza 		= $natureza;
					$credor->contexto->td_moeda 		= $moeda;
					...
					$credor->contexto->td_numerorelacao	= 1;

					$credor->contexto->td_processo		= (int)$processo[1];
					$credor->contexto->td_origemcredor	= $origemcredor;
					$credor->contexto->farein			= (int)$processo[0];
					$credor->contexto->td_numerorelacao	= $numerorelacao;
					...
					if (isset($_POST["credoresestrangeiro"])) { // Adiciona na lista caso seja estrangeiro
						...
						$lista->entidade_pai 	= $processo[2];
						$lista->entidade_filho 	= 20;
						$lista->reg_pai 		= $processo[0];
						$lista->reg_filho 		= $idCredor;
			...
			if (!isset($_POST["credoresestrangeiro"])) { // Não pode conciliar com CNPJ/CPF os credores estrangeiro
				// Concilia CNPJ e CPF
				$sql = "select *,sum(valor) soma from td_relacaocredores_temp where farein = ".(int)$processo[0]." and td_origemcredor = {$origemcredor} and (cpf is not null or cnpj is not null) group by td_classificacao,cnpj,cpf";
				...
				while ($linha = $query->fetch()){
					...
					$credor->contexto->td_tipo 			= $linha["td_tipo"];
					$credor->contexto->cnpj 			= substr($linha["cnpj"],0,1)=="#"?"null":$linha["cnpj"];
					$credor->contexto->cpf				= substr($linha["cpf"],0,1)=="#"?"null":$linha["cpf"];
					$credor->contexto->td_classificacao = $linha["td_classificacao"];
					$credor->contexto->td_natureza 		= $linha["td_natureza"];
					$credor->contexto->td_moeda 		= $linha["td_moeda"];
	...
	foreach ($dataset as $dado){
		...
		$op->value = $dado->id . "^" . $dado->td_processo . "^16";
	...
	foreach ($dataset as $dado){
		$op = tdClass::Criar("option");
		$op->value = $dado->id . "^" . $dado->td_processo . "^19";
		$op->add("[ ".completaString($dado->id,3)." ][ ".($dado->cnpj==""?$dado->cpf:$dado->cnpj)." ] - " . $dado->razaosocial);
		$select_processo->add($op);
	}
	...
	foreach ($dataset as $dado){
		$op = tdClass::Criar("option");
		$op->value = $dado->id . "^" . $dado->td_processo . "^18";
		$op->add("[ ".completaString($dado->id,3)." ][ ".($dado->cnpj==""?$dado->cpf:$dado->cnpj)." ] - " . $dado->razaosocial);
		$select_processo->add($op);
	}
	...
	$span_modelo->class 	= "glyphicon glyphicon-file";
