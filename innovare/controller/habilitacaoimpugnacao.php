	...
	if (isset($_POST["op"])){
		...
		if ($_POST["op"] == "salvar_parecer"){			
			if ($conn = Transacao::get()){
				...
				$obj->contexto->td_habilitacaoimpugnacao = $_POST["id"];
				$obj->contexto->armazenar();
				$conn->commit();
			}			
			exit;
		...
		if ($_POST["op"] == "carregar_lista_parecer"){
			...
			$sql->addFiltro("td_habilitacaoimpugnacao","=",$_POST["id"]);
		...
		if ($_POST["op"] == "salvar_habilitacaoimpugnacao"){
			if ($conn = Transacao::get()){
				...
				if ($_POST["credor"] == ""){
					...
					$credor->id = $idCredor;
					$credor->td_processo = $_POST["processo"];
					$credor->farein = 	$_POST["farein"];
					$credor->td_tipo = $_POST["tipopessoa"];
					$credor->td_tipoempresa = $_POST["tipoempresa"];				
					...
					$credor->td_estado = 	$_POST["estado"];
					$credor->td_origemcredor = 6;
					$credor->td_classificacao = $_POST["classificacao"];
					$credor->td_moeda = $_POST["moeda"];
					$credor->td_natureza = $_POST["natureza"];
					...
					$idCredor = $credor->getUltimo() + 1;
					$credor->id = $idCredor;				
					$credor->td_processo = $credorOLD->td_processo;
					$credor->farein = $credorOLD->farein;
					$credor->td_tipo = $credorOLD->td_tipo;
					$credor->td_tipoempresa = $credorOLD->td_tipoempresa;
					if ($credorOLD->td_tipo == 1){
					...
					$credor->td_estado = $credorOLD->td_estado;
					$credor->td_origemcredor = 7;
					$credor->td_classificacao = $credorOLD->td_classificacao;
					$credor->td_moeda = $credorOLD->td_moeda;
					$credor->td_natureza = $credorOLD->td_natureza;
				...
				$habilitacaoimpugnacao->td_processo = $_POST["processo"];
				$habilitacaoimpugnacao->td_magistrado = $_POST["magistrado"];
				$habilitacaoimpugnacao->td_comarca = $_POST["comarca"];
				$habilitacaoimpugnacao->data = date("Y-m-d");
				$habilitacaoimpugnacao->td_credor = $idCredor;
				$habilitacaoimpugnacao->td_credorimpugnado = $_POST["credor"];
...
';
	...
	$js->add('
		...
		$("#btn-impugnacao").click(function(){
			...
			pesquisaCredor.addFiltro("td_origemcredor","=",4);
