<?php
	if (isset($_GET["op"])){
		if ($_GET["op"] == "add_bairro"){
			...
			$entidade->contexto->td_cidade = (int)$_GET["cidade"];
				...
				$(".retornar-endereco").click(function(){				
					...
					$.ajax({
						...
						success:function(retorno){
							...
							$("#pais").val(ret[0]);
							$("#estado").load("index.php?controller=requisicoes&op=carregar_options&entidade=19&filtro=td_pais^" + ret[0] + "&valor=" + ret[1],
								function(){
									$("#cidade").load("index.php?controller=requisicoes&op=carregar_options&entidade=20&filtro=td_estado^" + ret[1] + "&valor=" + ret[2],
										function(){																				
											if (ret[3] != ""){
												$("#bairro").load("index.php?controller=requisicoes&op=carregar_options&entidade=21&filtro=td_cidade^" + ret[2] + "&valor=" + ret[3]);											
											}else{
...
<script type="text/javascript">
	...
	function addBairro(cidade,bairro){
		...
		$.ajax({
			...
			success:function(retorno){
				$("#bairro").load("index.php?controller=requisicoes&op=carregar_options&entidade=21&valor=" + retorno + "&filtro=td_cidade^" + cidade,
				function(){
	...
	$("#pais").change(function(){
		...
		$("#estado").load("index.php?controller=requisicoes&op=carregar_options&entidade=8&filtro=pais^" + this.value,
			function(){
				...
				$("#cidade").load("index.php?controller=requisicoes&op=carregar_options&entidade=7&filtro=td_estado^" + $("#estado option:first-child").val(),					
					function(){
						$(this).prop("disabled",false);						
						$("#bairro").load("index.php?controller=requisicoes&op=carregar_options&entidade=12&filtro=td_cidade^" + $("#cidade option:first-child").val(),
							function(){
								$(this).prop("disabled",false);
	...
	$("#estado").change(function(){
		...
		$("#cidade").load("index.php?controller=requisicoes&op=carregar_options&entidade=7&filtro=td_estado^" + this.value,
			function(){
				$(this).prop("disabled",false);				
				$("#bairro").load("index.php?controller=requisicoes&op=carregar_options&entidade=12&filtro=td_cidade^" + $("#cidade option:first-child").val(),
					function(){
	...
	$("#cidade").change(function(){
		...
		$("#bairro").load("index.php?controller=requisicoes&op=carregar_options&entidade=12&filtro=td_cidade^" + this.value,
			function(){
				$(this).prop("disabled",false);				
	...
	$(document).ready(function(){		
		$("#estado").load("index.php?controller=requisicoes&op=carregar_options&entidade=19&filtro=td_pais^1&valor=1",
			function(){				
				$("#cidade").load("index.php?controller=requisicoes&op=carregar_options&entidade=20&filtro=td_estado^1&valor=1",
					function(){
						$("#bairro").load("index.php?controller=requisicoes&op=carregar_options&entidade=21&filtro=td_cidade^1",
							function(){
