<?php
	...
	/*
		...
		if ($linha = $query->fetch()){
			...
			if ($tipo_farein == "RE"){ // Recupera&ccedil;&atilde;o
				...
				$sql = "SELECT razaosocial FROM td_recuperanda WHERE td_processo = $processo AND id={$farein}";
				$query = $conn->query($sql);
				if ($linha = $query->fetch()){
				...
				$sql = "SELECT razaosocial FROM td_falencia WHERE td_processo = $processo AND id={$farein}";
				$query = $conn->query($sql);
				if ($linha = $query->fetch()){
					$razaosocial =  $linha["razaosocial"];
	...
	*/	
		...
		$sql = "SELECT a.id,a.descricao FROM td_classificacao a WHERE exists(SELECT 1 FROM td_relacaocredores b WHERE b.td_classificacao = a.id AND b.td_processo = $processo AND b.farein = {$farein} AND b.td_origemcredor  = 4) ORDER BY a.ordem ASC ";
		...
		while ($linha = $query->fetch()){
			...
			$sql_credores = "SELECT nome,cpf,cnpj,valor,td_tipo,td_moeda FROM td_relacaocredores WHERE td_processo = $processo AND farein = $farein AND td_origemcredor = 4 AND td_classificacao = {$linha["id"]} ORDER BY nome ";
...
echo '	<center>
			...
			<table  cellspacing = "0" width="80%">
				...
				'.($totalEuro>0?'<tr>
					<td width="70%" style="border:1px solid #000;font-weight:bold;">TOTAL ( EUR&#8364; )</td><td width="30%" style="text-align:right;border:1px solid #000;">EUR&#8364; '.moneyToFloat($totalEuro,true).'</td>
				</tr>':'').'				
			</table>
		</center>';
?>