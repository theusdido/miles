...
error_reporting(E_ALL);
	...
	if (isset($_POST["op"])){
		...
		if ($_POST["op"] == "excluir"){
			...
			$credor = tdClass::Criar("persistent",array("td_relacaocredores",$dv->contexto->td_credor));
		...
		if ($_POST["op"] == "enviar_parecer"){
			...
			if ($conn = Transacao::get()){
				...
				$mail->Subject = utf8_decode("Decisão Proferida");
		...
		if ($_POST["op"] == "salvar_parecer"){		
			if ($conn = Transacao::get()){
				...
				$obj->contexto->td_classificacao = $_POST["parecerclassificacao"];
				$obj->contexto->td_moeda = $_POST["parecermoeda"];
				if ($_POST["legitimidade"]!=""){
					$obj->contexto->legitimidade = $_POST["legitimidade"];	
				}				
				$obj->contexto->projeto = 1;
				$obj->contexto->empresa = 1;
				$obj->contexto->td_habilitacaodivergencia = $_POST["id"];
				$obj->contexto->armazenar();
				$conn->commit();
			}			
			exit;
		...
		if ($_POST["op"] == "carregar_lista_parecer"){
			$sql = tdClass::Criar("sqlcriterio");
			$sql->addFiltro("td_habilitacaodivergencia","=",$_POST["id"]);
			$dataset = tdClass::Criar("repositorio",array("td_habilitacaodivergenciaparecer"))->carregar($sql);
			foreach($dataset as $d){
				echo '	<tr>
							<td>'.$d->legitimidade.'</td>
							<td>'.utf8_encode(tdClass::Criar("persistent",array("td_classificacao",$d->td_classificacao))->contexto->descricao).'</td>
							<td>'.utf8_encode(tdClass::Criar("persistent",array("td_moeda",$d->td_moeda))->contexto->descricao).'</td>
	...
	if (isset($_GET["op"])){
		...
		if ($_GET["op"] == "transferirprotocoloprocesso"){
			...
			$credor		= $protocolo->td_credor;
			$dados		= explode("^",$_GET["farein"]);
			$farein 	= $dados[0];
			$processo 	= $dados[1];

			$conn = Transacao::Get();
			$conn->exec("UPDATE td_habilitacaodivergencia SET td_processo = {$processo} WHERE id = {$protocolo->id};");
			$conn->exec("UPDATE td_relacaocredores SET td_processo = {$processo},farein = {$farein} WHERE id = {$credor};");
			Transacao::Fechar();
			echo 1;
			exit;
		}
		...
		if ($_GET["op"] == "alterarrelacao"){
			...
			$credorid 		= tdClass::Criar("persistent",array("td_habilitacaodivergencia",$protocolo))->contexto->td_credor;
			
			$conn 		= Transacao::Get();
			$instrucao 	= "UPDATE td_relacaocredores SET td_numerorelacao = {$numerorelacao} WHERE id = {$credorid};";
		...
		if ($_GET["op"] == "converter_parecer"){
			...
			foreach($data_divhab as $d){
				...
				if ($d->td_parecerclassificacao != ""){
					$parecer->td_classificacao = $d->td_parecerclassificacao;
				}
				if ($d->td_parecermoeda != ""){
					$parecer->td_moeda = $d->td_parecermoeda;
				}
				
				$parecer->legitimidade = "";
				$parecer->projeto = 1;
				$parecer->empresa = 1;
				$parecer->td_habilitacaodivergencia = $d->id;
				$parecer->armazenar();
			}
			Transacao::fechar();
			exit;
		...
		if ($_GET["op"] == "transferencia"){
			...
			if ($conn = Transacao::get()){
				...
				$sqlHab->addFiltro("td_processo","=",$_GET["processo"]);
				$dataset = tdClass::Criar("repositorio",array("td_habilitacaodivergencia"))->carregar($sqlHab);
				foreach($dataset as $data){
					$opt = tdClass::Criar("option");
					$opt->value = $data->id;
					$credor = tdClass::Criar("persistent",array("td_relacaocredores",$data->td_credor))->contexto;
					if ($credor->td_origemcredor == 3){
				...
				$sqlDiv->addFiltro("td_processo","=",$_GET["processo"]);
				$dataset = tdClass::Criar("repositorio",array("td_habilitacaodivergencia"))->carregar($sqlDiv);
				foreach($dataset as $data){
					$opt = tdClass::Criar("option");
					$opt->value = $data->id;
					$credor = tdClass::Criar("persistent",array("td_relacaocredores",$data->td_credor))->contexto;
					if ($credor->td_origemcredor == 2){					
						$opt->add(completaString($data->id,5) . " - " . tdClass::Criar("persistent",array("td_relacaocredores",$data->td_credor))->contexto->nome);
			...
			$spanHabToDiv->class = "glyphicon glyphicon-arrow-right";
			...
			$spanDivToHab->class = "glyphicon glyphicon-arrow-left";
		...
		if ($_GET["op"] == "transferir"){
			...
			$credor = tdClass::Criar("persistent",array("td_relacaocredores",$divhab->td_credor))->contexto;
			$credor->td_origemcredor = $_GET["habdiv"];
			$credor->armazenar();
			Transacao::fechar();	
			/*
	...
	if (isset($_GET["op"])){
		...
		if ($_GET["op"] == "carrega_divhab"){
			...
			$sql_habdiv->addFiltro("td_processo","=",$_GET["processo"]);
			...
			$where = " AND ( b.td_numerorelacao IN ($listarelacao) OR td_numerorelacao = 0 OR td_numerorelacao IS NULL )";

			$conn = Transacao::get();
			$sql2 = '
				SELECT b.td_origemcredor,IFNULL(b.td_numerorelacao,0) numerorelacao,a.*
				...
				'.$where.'
				ORDER BY b.td_origemcredor,numerorelacao ASC;
			';
			...
			while($habdiv = $query2->fetchObject()){
				...
				$td_credor->add(utf8_decode($msg_origem) . " - <b>[ " . $cpfj . " ]</b><br/>" . utf8charset($credor->contexto->nome));
				...
				$td_relacao->add('
					<div class="btn-group btn-group-xs" role="group" aria-label="...">
					  <button type="button" class="btn btn-default alterar-relacao '.($credor->contexto->td_numerorelacao==1?'active':'').'">1</button>
					  <button type="button" class="btn btn-default alterar-relacao '.($credor->contexto->td_numerorelacao==2?'active':'').'">2</button>
					  <button type="button" class="btn btn-default alterar-relacao '.($credor->contexto->td_numerorelacao==3?'active':'').'">3</button>
					</div>
				');
				...
				$formT->action = getURLProject("http://innovareadministradora.com.br/sistema/index.php?controller=gerarcapa&key=k&protocolo=" . $habdiv->id . "&farein=".$_GET["farein"] . "&credor=".$credor->contexto->id . "&opt=habdiv");
				$formT->method = "POST";
				$formT->target = "_blank";

				$formZ = tdClass::Criar("form");
				$formZ->action = getURLProject("http://innovareadministradora.com.br/sistema/index.php?controller=gerarzip&key=k&protocolo=" . $habdiv->id . "&farein=".$_GET["farein"] . "&credor=".$credor->contexto->id . "&opt=habdiv");
				$formZ->method = "POST";
					...
					$("#tr-arquivos-'.$habdiv->id.' .btn-reenvioarquivos").click(function(){
						...
						$.ajax({
							...
							data:{
								...
								op:"reenvioarquivo",
								opc:$(this).data("opc"),
								divergencia:$(this).data("divergencia"),
								currentproject:'.$_SESSION["currentproject"].'
							}
						});
					});
				...
				$iconDecisao->class = "glyphicon glyphicon-file";
			...
			');			
			$jsRelacao->mostrar();
			$table->add(utf8_decode("<tr><td colspan='10' align='right'>Divergências: <b>".$total_div."</b> - Habilitações: <b>".$total_hab."</b></td></tr>"));			
			$table->mostrar();
			exit;
	...
	$titulo->add(utf8_decode("Habilitação/Divergência de Crédito"));
	...
	foreach($dataset as $processo){
		...
		$descricaoProcesso = $processo->descricao!=''?' <i>{ '.utf8_decode($processo->descricao).' }</i>':'';
		$a = tdClass::Criar("hyperlink");
		...
		$btnTransferir->onclick = "abrirModalTransferencia({$processo->id});";
			...
			$btnTransferirSpan->class = "glyphicon glyphicon-transfer";
	...
	$js->add('
		...
		$(document).ready(function(){
			$(document).on("dblclick","#lista-divergencias,#lista-habilitacoes",function(){
				...
				carregar(session.urlsystem + "?controller=selectfarein","#div-farein",function(){
