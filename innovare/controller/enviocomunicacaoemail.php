<?php
	...
	if ($op == "criarpdf"){
		...
		$urlimpressao = Session::Get("URL_SYSTEM") . "index.php?controller=impressaocomunicacao&gerarpdf=true&codigo={$credor}&farein={$farein}&key=k&currentproject=" . $_SESSION["currentproject"];
	...
	if (tdClass::Read("op") == "carregacredoresids"){
		...
		$conn = Transacao::Get();
		$retorno = array();
		$sql = "SELECT id,nome,cnpj,cpf,farein,td_processo,email FROM td_relacaocredores WHERE farein = {$farein} AND td_origemcredor = 11  {$whereCodigo};";
		$query = $conn->query($sql);
		while ($linha = $query->fetch()){
			$processo = tdClass::Criar("persistent",array("td_processo",$linha["td_processo"]))->contexto;
