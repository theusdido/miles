	...
	if ($conn = Transacao::get()){		
		...
		if ($linha = $query->fetch()){
			...
			if ($linha["tipoprocesso"] == 16){ // Recupera&ccedil;&atilde;o
				...
				$sql = "SELECT razaosocial FROM td_recuperanda WHERE td_processo = $processo";
				$query = $conn->query($sql);
				if ($linha = $query->fetch()){
		...
		$sql = "SELECT a.id,a.descricao FROM td_classificacao a WHERE EXISTS(SELECT 1 FROM td_relacaocredores b WHERE b.td_classificacao = a.id AND b.td_processo = $processo AND (td_origemcredor is null OR td_origemcredor = 1)) ORDER BY a.ordem ASC;";
		$query = $conn->query($sql);
		$tr = "";
		$total = 0;
		while ($linha = $query->fetch()){
			...
			$sql_credores = "SELECT nome,cpf,cnpj,valor,td_tipo FROM td_relacaocredores WHERE td_processo = $processo AND td_classificacao = {$linha["id"]} AND (td_origemcredor is null OR td_origemcredor = 1) order by codigo";
			$query_credores = $conn->query($sql_credores);
			while ($linha_credores = $query_credores->fetch()){
				echo " " . strtoupper($linha_credores["nome"] . " - " . ($linha_credores["td_tipo"]==1?$linha_credores["cnpj"]:$linha_credores["cpf"]) . " - R$" . moneyToFloat($linha_credores["valor"],true) . ";");
				$valor += $linha_credores["valor"];
			}			
			$tr .= '<tr><td width="70%" style="border:1px solid #000;font-weight:bold;">'.$linha["descricao"].'</td><td width="30%" style="text-align:right;border:1px solid #000;">R$'.moneyToFloat($valor,true).'</td></tr>';
			$total += $valor;
		}
echo '
			</p>
...
echo '	<center>
			...
			<table  cellspacing = "0" width="80%">
				'.$tr.'
				<tr>
					<td width="70%" style="border:1px solid #000;font-weight:bold;">TOTAL</td><td width="30%" style="text-align:right;border:1px solid #000;">R$ '.moneyToFloat($total,true).'</td>
				</tr>	
			</table>
		</center>';
?>