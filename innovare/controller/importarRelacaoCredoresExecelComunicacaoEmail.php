	...
	if (isset($_FILES["arquivo"])){
		...
		if ($conn = Transacao::get()){
			...
			foreach ($xml->Worksheet->Table->Row as $cell){
				...
				$sqlExiste = "SELECT id FROM td_relacaocredores WHERE (replace(replace(replace(cnpj,'/',''),'-',''),'.','') = '$cpfj' or replace(replace(cpf,'-',''),'.','') = '$cpfj') AND (cpf <> '$cpfj' or cnpj <> '$cpfj') AND td_classificacao = {$classificacao} AND td_processo = {$processo[1]} AND farein = {$processo[0]} AND td_moeda = {$moeda} AND td_origemcredor = {$origemcredor}";
