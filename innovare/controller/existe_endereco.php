...
if (isset($_POST)){
	...
	if (isset($_POST["estado"]) && $pais != ""){
		...
		$sql_estado->addFiltro("td_pais","=",$pais);
	...
	if (isset($_POST["cidade"]) || $pais != "" && $estado != ""){
		if (is_numeric($_POST["cidade"])){
			...
			$sql_cidade2->addFiltro("td_estado","=",$estado);
	...
	if (isset($_POST["bairro"]) && $pais != "" && $estado != "" && $cidade != ""){
		if (is_numeric($_POST["bairro"])){
			...
			$sql_bairro2->addFiltro("td_cidade","=",$cidade);
	...
	if (isset($_POST["op"])){
		if ($_POST["op"] == "salvar_endereco"){
			...
			#$sql_endereco->addFiltro("td_cidade","=",$_POST["cidade"]);
			$sql_endereco->addFiltro("td_bairro","=",$_POST["bairro"]);
			...
			if ($endereco->quantia($sql_endereco) > 0){
				...
				if ($conn = Transacao::get()){
					...
					$sql = "INSERT INTO td_endereco (id,td_bairro,cep,logradouro) VALUES ({$prox[0]},{$_POST["cidade"]},'{$cep}','{$logradouro}');";
					$conn->exec($sql);				
					Transacao::fechar();
