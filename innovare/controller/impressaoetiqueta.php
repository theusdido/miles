	...
	<body>
		<?php
			if ($conn = Transacao::get()){
				...
				$sql = "SELECT * FROM td_relacaocredores WHERE td_processo = {$processo} and farein = {$farein} {$where_codigo} order by td_classificacao,nome asc";				
				...
				foreach ($result as $key => $value){
					...
					$estado			= tdClass::Criar("persistent",array("td_estado",$value["td_estado"]))->contexto->sigla;
					$cont_pagina++;
					if ($cont_pagina >= 8){
