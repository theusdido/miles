	...
	if ($conn = Transacao::get()){		
		...
		if ($linha = $query->fetch()){
			...
			if ($linha["tipoprocesso"] == 16){ // Recupera&ccedil;&atilde;o
				...
				$sql = "SELECT razaosocial FROM td_recuperanda WHERE td_processo = $processo and id={$_GET["recuperanda"]}";
				$query = $conn->query($sql);
				if ($linha = $query->fetch()){
				...
				$sql = "SELECT razaosocial FROM td_falencia WHERE td_processo = $processo";
				$query = $conn->query($sql);
				if ($linha = $query->fetch()){
					$razaosocial =  $linha["razaosocial"];
		...
		$sql = "SELECT a.id,a.descricao FROM td_classificacao a WHERE exists(SELECT 1 FROM td_relacaocredores b WHERE b.td_classificacao = a.id AND b.td_processo = $processo AND (td_origemcredor is null OR td_origemcredor = 1) AND b.farein = {$_GET["recuperanda"]}) ORDER BY a.ordem ASC ";
		$query = $conn->query($sql);
		$tr = "";
		$total = 0;
		...
		while ($linha = $query->fetch()){
			...
			$sql_credores = "SELECT nome,cpf,cnpj,valor,td_tipo,td_moeda FROM td_relacaocredores WHERE td_processo = $processo AND td_classificacao = {$linha["id"]} AND farein = {$_GET["recuperanda"]} AND (td_origemcredor is null OR td_origemcredor = 1 or td_origemcredor is null) GROUP BY cpf,cnpj ORDER BY nome";
			...
			while ($linha_credores = $query_credores->fetch()){
				...
				echo " " . strtoupper($linha_credores["nome"] . " - ". ($linha_credores["td_tipo"]==1?$linha_credores["cnpj"]:$linha_credores["cpf"]) . " - ".($simboloMoeda)." " . moneyToFloat($linha_credores["valor"],true) . ";");
