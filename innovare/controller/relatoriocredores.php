<?php
	...
	if (isset($_GET["farein"])){
		...
		// Corpo CSV
		$corpoCSV 		= array();
		$sql = "SELECT * FROM td_relacaocredores WHERE farein = {$farein} AND td_processo = {$processo} AND td_origemcredor = 1;";
		...
		while ($linha = $query->fetch()){
			...
			$sqlUF = "SELECT nome,td_pais FROM td_estado WHERE id = " . $linha["td_estado"];
			...
			$nomepais = "";
			if (is_numeric($linhaUF["td_pais"])){
				$sqlPais = "SELECT descricao FROM td_pais WHERE id = " . $linhaUF["td_pais"];
			...
			if (is_numeric($linha["td_classificacao"])){
				$sqlClassificacao = "SELECT descricao FROM td_classificacao WHERE id = " . $linha["td_classificacao"];
		...
		if ($op == "downloadexcel"){
			...
			$script->add('
				function downloadExcel(){
					window.open("'.$_SESSION["URL_SYSTEM"].'index.php?controller=relatoriocredores&farein='.$fareinSel.'&op=downloadexcel&currentproject='.Session::Get("projeto").'","iframe-excel");
				}
			');
			$script->mostrar();
	...
	$script->add('
		...
		$(document).ready(function(){
			carregar(session.urlsystem + "?controller=selectfarein","#busca_farein",function(){
