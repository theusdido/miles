	...
	if (!empty($_POST)){
		...
		if ($_POST["op"] == "salvaralterar"){
			...
			if ($id == ""){
				...
				$prox = $query_prox->fetch();
				$id = $prox[0];
				$sql = "INSERT INTO ".PREFIXO."movimentacaoalterar (id,".PREFIXO."atributo,".PREFIXO."movimentacao,legenda) VALUES ({$id},{$atributo},{$movimentacao},'{$legenda}');";
			}else{
				$sql = "UPDATE ".PREFIXO."movimentacaoalterar SET ".PREFIXO."atributo = {$atributo} , ".PREFIXO."movimentacao = {$movimentacao} , legenda = '{$legenda}' WHERE id = {$id};";
			}			
			$query = $conn->query($sql);
			if($query){
				//addLog($sql);
		...
		if ($_POST["op"] == "salvarstatus"){
			...
			if ($id == ""){
				...
				$prox = $query_prox->fetch();
				$id = $prox[0];
				$sql = "INSERT INTO ".PREFIXO."movimentacaostatus (id,operador,valor,".PREFIXO."atributo,".PREFIXO."movimentacao) VALUES ({$id},'{$operador}','{$valor}',{$atributo},{$movimentacao});";
			}else{
				$sql = "UPDATE ".PREFIXO."movimentacaostatus SET ".PREFIXO."atributo = {$atributo} , ".PREFIXO."movimentacao = {$movimentacao} , valor = '{$valor}' , operador = '{$operador}' WHERE id = {$id};";
			}
			$query = $conn->query($sql);
			if($query){
				//addLog($sql);				
		...
		if ($_POST["op"] == "salvarhistorico"){
			...
			if ($id == ""){
				...
				$prox = $query_prox->fetch();
				$id = $prox[0];
				$sql = "INSERT INTO ".PREFIXO."movimentacaohistorico (id,".PREFIXO."atributo,".PREFIXO."movimentacao,legenda) VALUES ({$id},{$atributo},{$movimentacao},'{$legenda}');";
			}else{
				$sql = "UPDATE ".PREFIXO."movimentacaohistorico SET ".PREFIXO."atributo = {$atributo} , ".PREFIXO."movimentacao = {$movimentacao} , legenda = '{$legenda}' WHERE id = {$id};";
			}
			$query = $conn->query($sql);
			if($query){
				//addLog($sql);		
											...
											<form id="form-status">
												<fieldset>
													...
													<div class="form-group">
														...
														<select id="atributo" name="atributo" class="form-control">
														<?php 
															$sql = "SELECT id,nome,descricao,chaveestrangeira,tipohtml FROM ".PREFIXO."atributo WHERE ".PREFIXO."entidade = " . $entidade;
															$query = $conn->query($sql);
															foreach($query->fetchAll() as $linha){
																if ($linha["tipohtml"] == 7){
																...
																echo '<option value="'.$linha["id"].'" data-nome="'.$linha["nome"].'"  '.$carregalista.'>'.utf8_encode($linha["descricao"]).' [ '.$linha["nome"].' ]</option>';
															}
														?>
														</select>
													</div>
