...
function GradeDeDados(entidade){
	...
	this.registroporcoluna = 3;
	this.apenasload = false;
...
GradeDeDados.prototype.setaTable = function(){
	...
	if ($(".gradededados[data-contexto='"+this.contexto+"']").length <= 0){		
		this.table = $("<div class='gradededados' data-contexto='"+this.contexto+"'></div>");
	}else{
...
GradeDeDados.prototype.show = function(){
	...
	if ($(this.contexto).find(".gradededados").length <= 0){
		if ($(".pesquisa-grade").length <= 0){
			this.pesquisa();
		}
		this.table.insertAfter(".pesquisa-grade");
		this.load();
	}else{
		this.reload();
	}
...
GradeDeDados.prototype.load = function(){
	...
	$.ajax({
		type:"POST",
		async:this.apenasload?false:true,
		...
		beforeSend:function(){
			...
			//loader(instancia.contexto);
		},
		dataType:"json",
		complete:function(ret){

			try{
				$(".gradededados[data-contexto='"+instancia.contexto+"']").html("");
			}catch(error){
			...
			if (instancia.apenasload){
				var dadosretorno = {
					dados:instancia.dadosCorpo,
					dadosreais:instancia.dadosReaisCorpo
				};
			}
...
GradeDeDados.prototype.corpo = function(){
	...
	if (this.table == undefined){
		console.log("Corpo da grade de dados não definido");
		return false;
	}	
	if (this.table.find("painel-registro").length > 0 && qtdeTempReg <= 0){
		this.table.html("");
	}
	if (this.dadosCorpo.length > 0 && this.dadosCorpo[0] != undefined && this.totalRegistroRetorno > 0){
		for (ln in this.dadosCorpo){
			...
			var nomes_cabecalho = [];
			var descricao_cabecalho = [];
			for(rd in linhas){
				var dadosrel = this.addDadosRelacionamento(linhas[rd].id);
				for (drl in dadosrel){
					nomes_cabecalho = this.attr_cabecalho_nome.concat(dadosrel[drl].cabecalho);
					descricao_cabecalho = this.attr_cabecalho_descricao.concat(dadosrel[drl].descricao);
					for (drlc in dadosrel[drl].dados){
						for (drlco in dadosrel[drl].dados[drlc]){
							for (at in dadosrel[drl].cabecalho){
								if (dadosrel[drl].cabecalho[at] != "id"){
									linhas[rd][dadosrel[drl].cabecalho[at]] = dadosrel[drl].dados[drlc][drlco][dadosrel[drl].cabecalho[at]];
								}
							}
						}
					}
			...
			for(l in linhas){
				...
				if (nomes_cabecalho.length > 0){
					for(n in nomes_cabecalho){
						dadosLinha[nomes_cabecalho[n]] = linhas[l][nomes_cabecalho[n]];
						if (linhasreais != undefined){						
							dadosReaisLinha[nomes_cabecalho[n]] = linhasreais[l][nomes_cabecalho[n]];
						}
					}
				}else{
					console.log(linhas[l]);
					console.log(Object.values(linhas[l]));
				}	
				this.addLinha(linhas[l].id,dadosLinha,dadosReaisLinha,nomes_cabecalho,descricao_cabecalho);
			}
		}
	}else{
		if (qtdeTempReg <= 0){
	...
	$(".gradededados .painel-registro").css("width",((100 / this.registroporcoluna)-2) + "%"); // Menos 2 do espaçamento
}
...
GradeDeDados.prototype.rodape = function(){
	if (!this.retornaFiltro){
		if (this.table == undefined){
			console.log('Rodapé não definido');
			return false;
		} 
		if (this.table.find("#footer").length <= 0){
			if (typeof qtdeTempRegistro === "undefined"){
				var qtdeTempReg = 1;
			}else{
				var qtdeTempReg = qtdeTempRegistro(this.nomeEntidade);
			...
			if (this.dadosCorpo.length > 0 || qtdeTempReg > 0){
				var tfoot = $("<div id='footer'>");
				var tr = $("<span>");			
				var td = $("<span colspan='"+(this.attr_cabecalho_nome.length+3)+"'>");
...
GradeDeDados.prototype.addLinha = function(id,linha,linhareal="",nomes_cabecalho="",descricao_cabecalho=""){
	var i = 0;
	var painel = $("<div idregistro="+id+" class='panel panel-default painel-registro'></div>");
	var painelhead = $("<div class='panel-heading'></div>");
	var painelbody = $("<div class='panel-body'></div>");
	//var tr = $("<tr idregistro="+id+">");
	//var tr = $("<div idregistro="+id+" class='panel panel-default'><div class='panel-body'></div></div>");
	var qtdeids = 0;

	for(n in nomes_cabecalho){
		var valor =  linha[nomes_cabecalho[n]];
		var valorreal = linhareal[nomes_cabecalho[n]];
		var idAtributo = getIdAtributo(nomes_cabecalho[n],this.nomeEntidade);
		...
		if (nomes_cabecalho[n] == 'id'){
			qtdeids++;
			if (qtdeids > 1){
				continue;
			}
		}
		var gilabel = (descricao_cabecalho[n] == '' || descricao_cabecalho[n] == undefined ) ? '' : descricao_cabecalho[n].substr(0,8);
		var gidado = (valor == undefined || valor == '') ? '' : valor.substr(0,30);
		var td = $("<span class='grade-info'><span class='gi-label' title='"+descricao_cabecalho[n]+"'>"+gilabel+": </span><span class='gi-dado'>" + gidado + "</span></span>");
		painelbody.append(td);
	}
	
	var instancia = gradesdedados[this.contexto];
	var btnOutraJanela = "<span align='center'><span onclick=outrajanela(this,38978,'index.php?controller=crud&amp;id=38978&amp;op=add&amp;t=19&amp;filtro_rel_nn=&amp;modal=true'); class='botao fas fa-external-link-alt btn btn-default'></span></span>";
	...
	if (this.movimentacao != ""){
		var btnMovimentacao = $("<span align='center' class='editar-coluna-gradededados'></span>");
		btnMovimentacao.append(spanMovimentacao);
	}
	
	var spanEditar = $("<span reg='"+id+"' data-entidade='"+this.entidade+"' class='botao fas fa-edit btn btn-default btn-sm' ></span>");
	...
	var btnEditar = $("<span align='center' class='editar-coluna-gradededados'></span>");
	btnEditar.append(spanEditar);
	
	var tdExcluir = $("<span align='center' class='excluir-coluna-gradededados'>");
	var btnExcluir = $("<span class='botao fas fa-trash-alt btn btn-danger btn-sm'></span>");	
	...
	var checkExcluir = "<span align='center' class='checkexcluir-coluna-gradededados'><span align='center' class='grade-info'><input type='checkbox' value='"+id+"'></span></span>";
	
	var tdDuplicar = $("<span align='center' class='duplicar-coluna-gradededados'>");
	var btnDuplicar = $("<span class='botao fas fa-clone btn btn-default btn-sm'></span>");	
	
	btnDuplicar.click(function(){
		instancia.duplicar(id,$(this));
	});	
	
	tdDuplicar.append(btnDuplicar);
	

	var tdImprimir = $("<span align='center' class='imprimir-coluna-gradededados'>");
	var btnImprimir = $("<span class='botao fas fa-print btn btn-default btn-sm'></span>");	
	
	btnImprimir.click(function(){
		window.open("index.php?controller=imprimeregistro&entidade=" + entidade + "&registro=" + id);
	});	
	
	tdImprimir.append(btnImprimir);

	if (!this.retornaFiltro){
		if (this.movimentacao != ""){
			painelhead.append(btnMovimentacao);
		}
		painelhead.append(checkExcluir);
		painelhead.append(tdExcluir);
		painelhead.append(btnEditar);
		painelhead.append(tdDuplicar);
		painelhead.append(tdImprimir);
	}else{
		painelhead.attr("style","cursor:pointer;");
		//tr.attr("onclick",this.nomeEntidade + "GD.retornaid("+dados[0]+");");
		var instancia = this;
		painelhead.click(function(){
			instancia.retornaid(id); 
		});
	}
	
	painel.append(painelhead);	
	painel.append(painelbody);	
	//this.table.find("tbody tr[class=warning]").remove();
	this.table.append(painel);
...
GradeDeDados.prototype.addDadosRelacionamento = function(registro){
	var retorno = [];
	for(r in td_relacionamento){
		if (td_relacionamento[r].pai == this.entidade){

			var gdRel = new GradeDeDados(td_relacionamento[r].filho);
			gdRel.apenasload = true;
			gdRel.load();
			retorno.push({
				cabecalho:gdRel.attr_cabecalho_nome,
				descricao:gdRel.attr_cabecalho_descricao,
				dados:gdRel.dadosCorpo,
				dadosreais:gdRel.dadosReaisCorpo
			});
		}
	}
	return retorno;
...
GradeDeDados.prototype.duplicar = function(id,obj){
	var instancia = this;
	$.ajax({
		url:"index.php",
		data:{
			controller:"duplicarregistro",
			id:id,
			entidade:this.entidade
		},
		complete:function(ret){
			...
			bootbox.confirm({
				message: "Novo registro criado [ <b> "+retorno.idclone+" </b> ]. Deseja editá-lo ?",
				buttons: {
					confirm: {
						label: 'Sim',
						className: 'btn-success'
				...
				callback: function (result) {
					if (result){
						editarFormulario(instancia.entidade,retorno.idclone);
						//instancia.editar(retorno.idclone);
						//instancia.addFiltro("id","=",retorno.idclone);
						//instancia.reload();
						//instancia.editar(retorno.idclone);
						//$(".editar-coluna-gradededados .botao[reg="+retorno.idclone+"]").click();
					}else{
