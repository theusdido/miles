<?php
	...
	if (!file_exists(Session::Get("PATH_CURRENT_FILE_TEMP"))){
		 mkdir(Session::Get("PATH_CURRENT_FILE_TEMP"));
		 exit;
	}
	...
	if (isset($_GET["excluir"])){
		if (file_exists(Session::Get("PATH_CURRENT_FILE_TEMP") . $_GET["excluir"])){
			unlink(Session::Get("PATH_CURRENT_FILE_TEMP") . $_GET["excluir"]);
		}
	}
	...
	if (isset($_FILES[$id_input])){
		...
		$retorno 			= getHTMLTipoFormato(19,$valor,$atributo->contexto->td_entidade,$atributo->contexto->id);
		$nomearquivo 		= session_id() . "-" . $atributo->contexto->nome . "-" . $atributo->contexto->{ENTIDADE} . "-" . $idregistro . "." . $extensao;
	...
	if ($valor!=""){
		...
		if (!$isnew){
			...
			$button_up_deletar->onclick = "parent.excluirArquivoUpload(".str_replace('"',"'",getHTMLTipoFormato(19,$nomeexibicaoarquivo,$atributo->contexto->td_entidade,$atributo->contexto->id)).",{$entidade->contexto->id},{$atributo->contexto->id});";
			
			$button_up_visualizar = tdClass::Criar("button");
			$button_up_visualizar->class = "btn btn-default visualizar-td-upload";
			$srcRetorno = isset($retorno["src"])?$retorno["src"]:Session::Get("URL_CURRENT_FILE").$nome_arquivo_completo;
			$srcFile = file_exists(Session::Get("PATH_CURRENT_FILE").$nome_arquivo_completo)?Session::Get("URL_CURRENT_FILE").$nome_arquivo_completo:$srcRetorno;
			$icon_up = tdClass::Criar("span");
