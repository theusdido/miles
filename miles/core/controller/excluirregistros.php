	...
	if ($entidadeID != "" && $registrosID != ""){
		...
		if (is_numeric($t)){	
			...
			if ($conn = Transacao::get()){
				...
				//Excluindo arquivos 
				$sqlVArquivos = "SELECT nome FROM td_atributo WHERE td_entidade = {$entidade->contexto->id} and tipohtml in (19);";
				...
				// Excluindo os relacionamentos (LISTA)
				$sqlLista = "SELECT * FROM ".LISTA." WHERE entidadepai = AND entidadefilho = AND regpai =";
