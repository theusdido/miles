<?php
	$pathAngularRoot 	= '/cedupcriciuma.com.br/cedupcriciuma/';
	$isAngularJS 		= true;
	...
	$isprincipal = isset($_GET["principal"]) ? true : false;
	...
	// Relacionamentos
	if (tdClass::Read("relacionamento") != ""){
	...
	if ($entidade->contexto->exibirlegenda == 1){
		...
		$titulo->add(utf8charset($entidade->contexto->descricao,7));
	...
	if ($isprincipal){
		...
		if ($entidade->contexto->carregarlibjavascript == 1){
			...
			$jsGrade->src = PATH_TDC."gradededados.class.js";
			#$bloco->add($jsGrade);
