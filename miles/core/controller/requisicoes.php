...
switch($op){
	...
	case "retorna_descricao_filtro":
		...
		if ($atributo->contexto->chaveestrangeira == "" || $atributo->contexto->chaveestrangeira == 0){
			...
			$attr = tdClass::Criar("persistent",array(ENTIDADE,$atributo->contexto->chaveestrangeira));
			if ($conn = Transacao::get()){
				if ($attr->contexto->campodescchave == ""){
					// Pega o primeiro campo que é utilizado na grade de dados
					$sql_cd = tdClass::Criar("sqlcriterio");			
					$sql_cd->addFiltro(ENTIDADE,'=',$attr->contexto->id);
					$sql_cd->addFiltro("exibirgradededados",'=',1);
					$dataset_cd = tdClass::Criar("repositorio",array(ATRIBUTO))->carregar($sql_cd);
					if (!empty($dataset_cd)){
						$campodescchave = $dataset_cd[0]->id;
					...
					$campodescchave = $attr->contexto->campodescchave;
				}

				if ($attr->contexto->nome != null){
					$atributo 		= tdClass::Criar("persistent",array(ATRIBUTO,$campodescchave));
					$entidade 		= tdClass::Criar("persistent",array($attr->contexto->nome,$_GET["termo"]));
					$valorfinal 	= $entidade->contexto->{$atributo->contexto->nome};
					echo $valorfinal;
				}else{
					echo "Sem Descrição";
	...
	case "carregar_options":
		...
		foreach ($dataset as $dado){
			...
			echo '<option value="'.$dado->id.'" '.$selecionado.'>'.utf8charset($dado->{$campo_descricao}).'</option>';
		}		
	break;
	...
	case "alterproject":
		...
		if (tdInstall::isInstalled($database)){
			...
			setCurrentConfigFile(
				array(
					'PROJETO_DESC'				=> $projetocurrent->nome,
					'PROJETO_FOLDER'			=> 'miles/sistema',
					'RAIZ'						=> '',
					'PATH_LOGO_PADRAO'			=> 'tema/padrao/logo.png',
					'PATH_IMG_RODAPE_PADRAO' 	=> 'tema/padrao/rodape.png',
					'PREFIXO'					=> 'td',
					"CURRENT_PROJECT" 			=> $projetocurrent->projectdiretorio,
					'CURRENT_DATABASE' 			=> $tipo,
					'CODIGOCLIENTE'				=> $projetocurrent->id,
					'THEME'						=> 'padrao'
				)
			);
