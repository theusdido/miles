	...
	if ($_controller == "autentica"){
		...
		if (($login != '') && ($senha != '')){
			...
			if (sizeof($dataset) > 0){
				...
				$_usergroup		= $dataset[0]->grupousuario;
				...
				Session::append('usergroup'					,$_usergroup);

				if ($_usergroup != 1 && $_usergroup != 2){
					$_is_embedded = true;					
				}
				Session::append('is_embedded',$_is_embedded);
