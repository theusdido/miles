<?php
    ...
    function sendSignedLoteRps(
        ...
        if (!isset($_GET['nfsenumero'])) {
            echo json_encode(array(
                'is_exist_nfse' => 0,
                'message_error' => 'Número da NFS-e não informado.'
            ));
            exit;
        }

        $nfse_numero = $_GET['nfsenumero'];
        $codigo_cancelamento = $_GET['codigocancelamento'] ?? 1;
    ...
    try {
        ...
        if (!empty($resp['bodyNodes'])) {
            foreach ($resp['bodyNodes'] as $k => $v) {
                ...
                #var_dump($respXML);
                #exit;
                #var_dump($respXML->return[0]);

                $respNFSE = new SimpleXMLElement($respXML->return[0]);

                $is_exist_nfse = isset($respNFSE->ListaMensagemRetorno->MensagemRetorno) ? 1 : 0;
                $msg_retorno = 'Cancelamento realizado com sucesso.';

                if (isset($respNFSE->ListaMensagemRetorno->MensagemRetorno)){
                    $message_retorno = $respNFSE->ListaMensagemRetorno->MensagemRetorno;
                    $codigo_error = $message_retorno->Codigo;
                    $descricao_error = $message_retorno->Mensagem;
                    $msg_retorno = "Erro: $codigo_error - $descricao_error";
                }

                //var_dump($respNFSE->ListaMensagemRetorno->MensagemRetorno);

                echo json_encode(array(
                    'message' => $msg_retorno
                ));
                #var_dump(isset($respNFSE->CompNfse));
                #echo $respNFSE->CompNfse->Nfse->InfNfse->Numero;
                #var_dump(gettype($respXML->return[0]));
                #var_dump(gettype($respXML->return->consultarnfserpsresponse));
                #var_dump($respXML->return->consultarnfserpsresponse);
                #echo $respXML->consultarnfserpsresponse;
            }
        } else {
            #echo $resp['raw'];
        }
