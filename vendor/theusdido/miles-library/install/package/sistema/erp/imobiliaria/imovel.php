<?php
	// Setando variáveis
	$entidadeNome = "erp_imobiliaria_imovel";
	...
	// Capa
	$filial = criarAtributo($conn,$entidadeID,"filial","Filial","int",0,0,4,0,installDependencia($conn,'empresa','system/'),0,"",1,0);
	$empreendimento = criarAtributo($conn,$entidadeID,"empreendimento","Empreendimento","int",0,1,4,0,installDependencia($conn,'erp_imobiliaria_empreendimento'),0,"",1,0);
	$administradoracondominio = criarAtributo($conn,$entidadeID,"administradoracondominio","Administradora Condominio","int",0,1,4,0,installDependencia($conn,'erp_imobiliaria_administradoracondominio'),0,"",1,0);		
	$tipoimovel = criarAtributo($conn,$entidadeID,"tipoimovel","Tipo","int",0,0,4,0,installDependencia($conn,'erp_imobiliaria_tipoimovel'),0,"",1,0);	
	$tipopiso = criarAtributo($conn,$entidadeID,"tipopiso","Tipo de Piso","int",0,1,4,0,installDependencia($conn,'erp_imobiliaria_tipopiso'),0,"",1,0);	
	$mobiliado = criarAtributo($conn,$entidadeID,"mobiliado","Mobiliado ?","int",0,1,4,0,installDependencia($conn,'erp_imobiliaria_mobiliado'),0,"",1,0);
	...
	$proprietariosAba = installDependencia($conn,"erp_imobiliaria_imovelproprietario");
	$enderecosAba = installDependencia($conn,"erp_imobiliaria_imovelendereco");
	$fotoAba = installDependencia($conn,"erp_imobiliaria_imovelfoto");
