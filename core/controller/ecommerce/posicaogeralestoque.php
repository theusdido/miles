<?php	
	...
	$conn = Transacao::Get();
	$sql = "SELECT 1 FROM td_erp_material_posicaogeralestoque {$where} LIMIT 1";
	$query = $conn->query($sql);

	if ($query->rowCount() > 0){
		$sqlUpdate = "UPDATE td_erp_material_posicaogeralestoque SET saldo = (saldo " . $operacao . " " . $quantidade . ") , datahora = now() {$where} ";
		$queryUpdade = $conn->query($sqlUpdate);
	}else{
		$sqlInsert = "INSERT INTO td_erp_material_posicaogeralestoque (id,produto,".($variacaoproduto != ""?$variacaoinsertcampo . ",":"")."saldo,datahora) VALUES (".getProxId("ecommerce_posicaogeralestoque",$conn).",{$produto},".($variacaoproduto != ""?$variacaoinsertvalor . ",":"")."{$quantidade},now());";
