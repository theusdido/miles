<?php
	if (isset($_data)){
		...
		if (($login!="") and ($senha!="")){
			...
			$sqlCriterio1->add(tdClass::Criar("sqlfiltro",array("email",'=',$login)));
			...
			if ($dataset){
				...
				*/
					...
					if (!isset($ds_cliente[0])){
						...
						// Cliente
						$cliente_id = $ds_cliente[0]->regpai;						
						$cliente 	= tdc::rua('td_ecommerce_cliente', $cliente_id);

						// Seta o cliente ao carrinho de compras
						#$ecommerce_carrinho->setClient($cliente_id);

						// Token
						$_SESSION['token_access']   = md5(date('YmdHis') . $cliente_id);
