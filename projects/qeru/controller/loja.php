<?php
	switch($op){
        ...
        case 'get':
            ...
            $retorno = array_merge($loja_dados, array( 'endereco' =>  $endereco));
        ...
        case 'excluir-loja-topo':
            ...
            $filename	= PATH_CURRENT_FILE . 'bannertopo-'. getEntidadeId('td_ecommerce_loja') . '-' . $loja->id . "." . getExtensao($logo_name);
            if (file_exists($filename)) unlink($filename);
        break;        
        case 'page':
            $loja_id    = $dados['loja'];
