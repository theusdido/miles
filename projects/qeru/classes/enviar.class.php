<?php
	...
	class Enviar
	{
		...
		public function send()
		{
			...
			$mail->Host 			= $this->host;
			$mail->Port 			= $this->port;
			$mail->FromName 		= utf8_decode($this->from_name);

			$mail->SetFrom($this->email,$this->from_name);
			$mail->IsSMTP();
			...
			$mail->Body 	= utf8_decode($this->getHeader() . $this->getBody() . $this->getFooter());

			return $mail->Send();
		}
		public function setHeader($nome = "",$titulo = "",$subtitulo = "")
		{
			$_logo		= '<img src="'.URL_ROOT.'"assets/img/logo/logo.png" width="200" />';
