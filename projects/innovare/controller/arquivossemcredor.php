	...
	<tbody>
		<?php
			if ($conn = Transacao::Get()){
				...
				$sql = "SELECT a.descricao,a.id,a.origem,a.nome,a.relacaocredores FROM td_arquivos_credor a WHERE NOT EXISTS (SELECT 1 FROM td_relacaocredores b WHERE a.relacaocredores = b.id) ORDER BY a.origem;";
				...
				while ($linha = $query->fetch()){
					...
					';
					echo '	<tr>
								<td>'.$link.' - [ '.$linha["relacaocredores"].' ]</td>
								<td>'.$processoLista.'</td>
								<td>'.$fareinLista.'</td>
								<td>'.$credorLista.'</td>
								<td>'.$origem.'</td>
