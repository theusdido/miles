<?php
	if (tdClass::Read("op") == "atualizar_config"){
		...
		$config_comunicacao->codigoservicoadicionalcorreios		= tdClass::Read("codigoservicoadicionalcorreios");
		$config_comunicacao->codigoservicocorreios 				= tdClass::Read("codigoservicocorreios");
	...
	$form->legenda->add(tdc::utf8("Comunicação de Falência e Recuperação aos Credores"));
	...
	$script->add('
		...
		function gerar(){
			var farein = $("#retorno_empresa").val();
		...
		$("#b-atualizar-config").click(function(){
			...
			$.ajax({
				...
				data:{
					...
					codigoservicoadicionalcorreios:$("#codigoservicoadicionalcorreios").val(),
					codigoservicocorreios:$("#codigoservicocorreios").val()
				},
	...
	$modalArDownload->is_btn_close_white = true;
	...
	$codigo_servico_correios 					= tdClass::Criar("div");
	$codigo_servico_correios->class 			= "coluna";
	$codigo_servico_correios->data_ncolunas 	= 3;	
	$codigo_servico_correios->add(
		Campos::TextoLongo("codigoservicocorreios","codigoservicocorreios","Código de Serviço ( Correios )",$config_comunicacao->codigoservicocorreios)
	);	

	$codigo_servico_adicional_correios 					= tdClass::Criar("div");
	$codigo_servico_adicional_correios->class 			= "coluna";
	$codigo_servico_adicional_correios->data_ncolunas 	= 3;	
	$codigo_servico_adicional_correios->add(
		Campos::TextoLongo("codigoservicoadicionalcorreios","codigoservicoadicionalcorreios","Código Adicional de Serviço ( Correios )",$config_comunicacao->codigoservicoadicionalcorreios)
	);
	...
	$span_atualizar_config->class 	= "fas  fa-check";
	...
	$form_configuracoes->add(
		$grupo_botoes,
		$numeroinicialmaximoetiqueta,
		$numerofinalmaximoetiqueta,
		$ultimonumeroetiqueta,
		$email,
		$indiceremessa,
		$codigo_servico_correios,
		$codigo_servico_adicional_correios
	);
	...
	$pagina->add($form_bloco,$script,$selecao,$abas_bloco,$modalArDownload);
	$pagina->mostrar();
