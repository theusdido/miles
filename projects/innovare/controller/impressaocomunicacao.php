...
<html>
	...
	<body>
		<?php
			...
			$where_origem = " origemcredor = 10 ";
			
				if ($gerarpdf){
					$where_origem = " origemcredor = 11";
					$where_codigo = " AND id = " . tdClass::Read("codigo");
				}
			
			$sql = "SELECT * FROM td_relacaocredores WHERE processo = {$processo} and farein = {$farein} {$where_codigo} {$where_classificacao} and ({$where_origem} OR origemcredor = NULL or origemcredor = '' or origemcredor is null) ORDER BY classificacao asc, nome ";
