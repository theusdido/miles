<?php
	if (isset($_POST["op"])){
		...
		if ($conn = Transacao::get()){
			...
			$sqlArquivosAssembleia = "SELECT id FROM td_relacaocredores a WHERE a.processo = $processo AND farein = $farein AND a.origemcredor = 4 AND EXISTS (SELECT 1 FROM td_arquivosassembleia b WHERE a.id = b.credor);";
			...
			$sqlExcluirCredoresRelacaoAdministrador = "DELETE FROM td_relacaocredores WHERE processo = $processo AND farein = $farein AND origemcredor = 4 " . $where_excluirCredoresArquivosEnviados;
			...
			while ($linha_credor = $query_credor->fetch()){
				...
				if ($linha_credor["origemcredor"] == 2 || $linha_credor["origemcredor"] == 3){
					$sql_divhab = "	SELECT id,parecervalor,parecermoeda,parecerclassificacao FROM td_habilitacaodivergencia WHERE credor = {$linha_credor["id"]} AND decisao = 1";
					$query_divhab = $conn->query($sql_divhab);
					while ($linha_divhab = $query_divhab->fetch()){
						
						$sqlParecer = "SELECT legitimidade,valor,moeda,classificacao,cpfj FROM td_habilitacaodivergenciaparecer WHERE habilitacaodivergencia = {$linha_divhab["id"]} AND valor > 0";
				...
				}else if($linha_credor["origemcredor"] == 1){
	...
	if (isset($_GET["idpk"])){
		...
		foreach($dsHabDiv as $habdiv){
			if ($habdiv->credor != null && $habdiv->credor != "" && is_numeric($habdiv->credor)){
				...
				//echo $habdiv->credor . " - ".$credor->contexto->id."<br/>";				
				if ($credor->contexto->farein == $pk_id[1] && ($credor->contexto->origemcredor == 2 || $credor->contexto->origemcredor == 3)){
		...
		$sqlCredores->addFiltro("origemcredor","=",1);
