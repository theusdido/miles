	...
	if (isset($_GET["op"])){
		...
		if ($_GET["op"] == "carrega_arqass"){
			...
			while($habdiv = $query2->fetchObject()){			
				...
				$td_data->add(datetimeToMysqlFormat($habdiv->data,true));
				...
				$formT->action = URL_SYSTEM . "index.php?controller=gerarcapa&key=k&protocolo=" . $habdiv->id . "&farein=". $credor->contexto->farein . "&credor=".$credor->contexto->id . "&opt=assembleia&currentproject=".$_SESSION["currentproject"];
				$formT->method = "POST";
				$formT->target = "_blank";
				...
				$formADDFile->action = URL_SYSTEM . "index.php?controller=arquivosassembleia&op=addfile&credor=" . $credor->contexto->id . "&habdiv=" . $habdiv->id . "&currentproject=" . $_SESSION["currentproject"];
				...
				$jsAddFile->add('
					...
					$("#'.$nomeBtnFileInput.'").change(function(){
						...
						setTimeout(function(){
							...
							$("#'.$nomeSpanFileNew.'").append(\'<button onclick="excluirArquivo(this)" data-id="\'+fileid+\'" data-nomea="\'+nomea+\'" class="list-group-item arquivo-link-assembleia-files" style="float:left;width:5%;cursor:pointer;"><span class="fas fa-trash" aria-hidden="true"></span></button>\');
						},2000);
					});
