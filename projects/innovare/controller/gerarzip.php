...
if ($op == "habdiv"){
	...
	if( $zip->open( $arquivozip , ZipArchive::CREATE )  === true){
		$sql = "SELECT * FROM td_arquivos_credor WHERE relacaocredores = {$credor}";
...
if ($op == "farein"){
	$sqlf = "SELECT * FROM td_habilitacaodivergencia WHERE processo = 34;";
	$queryf = $conn->query($sqlf);
		...
		if( $zip->open( $arquivozip , ZipArchive::CREATE )  === true){
			while ($linhaf = $queryf->fetch()){
				$credor = tdClass::Criar("persistent",array("td_relacaocredores",$linhaf["credor"]))->contexto;
				$sql = "SELECT * FROM td_arquivos_credor WHERE relacaocredores = " . $linhaf["credor"];				
				$query = $conn->query($sql);
				while ($linha = $query->fetch()){
						$arquivonome = retirarAcentos(($linha["descricao"]));
