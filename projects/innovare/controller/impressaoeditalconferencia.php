	...
	if ($conn = Transacao::get()){		
		...
		if ($linha = $query->fetch()){
			...
			if ($linha["tipoprocesso"] == 16){ // Recupera&ccedil;&atilde;o
				...
				$sql = "SELECT razaosocial FROM td_recuperanda WHERE processo = $processo";
				$query = $conn->query($sql);
				if ($linha = $query->fetch()){
		...
		$sql = "SELECT a.id,a.descricao FROM td_classificacao a WHERE EXISTS(SELECT 1 FROM td_relacaocredores b WHERE b.classificacao = a.id AND b.processo = $processo AND (origemcredor is null OR origemcredor = 1)) ORDER BY a.ordem ASC;";
		$query = $conn->query($sql);
		$tr = "";
		$total = 0;
		while ($linha = $query->fetch()){
			...
			$sql_credores = "SELECT nome,cpf,cnpj,valor,tipo FROM td_relacaocredores WHERE processo = $processo AND classificacao = {$linha["id"]} AND (origemcredor is null OR origemcredor = 1) order by codigo";
			$query_credores = $conn->query($sql_credores);
			while ($linha_credores = $query_credores->fetch()){
				echo " " . strtoupper($linha_credores["nome"] . " - " . ($linha_credores["tipo"]==1?$linha_credores["cnpj"]:$linha_credores["cpf"]) . " - R$" . moneyToFloat($linha_credores["valor"],true) . ";");
