...
function retornaDataColeta(){
	...
	if (isset($_POST["op"])){
		if ($_POST["op"] == "gerar_ar"){
			...
			$remessa->usuario = Session::get()->userid;
			...
			$remessa->processo = tdClass::Read("processo");
			...
			foreach($credores_remessa as $c){
				...
				$comunicacaocredor->remessa = $remessa->numeroremessa;
	...
	if (isset($_GET["farein"])){
		...
		$conn = Transacao::get();
		$result = $conn->query("SELECT * FROM processo WHERE id ={$processo}");		
		$processo = "";
		foreach ($result as $key => $value){
			$processo = $value["id"];
			$numero = $value["numeroprocesso"];
			$dataenvio = dataExtenso(date("d/m/Y"));
			$juizo = (tdClass::Criar("persistent",array("td_juizo",$value["juizo"]))->contexto->descricao);
			$comarca = tdClass::Criar("persistent",array("td_comarca",$value["comarca"]))->contexto->descricao;
			...
			if ($tipo_farein=="FA"){							
				foreach ($resul_falida as $falida){					
					...
					$cidade = tdClass::Criar("persistent",array("td_cidade",$falida["cidade"]))->contexto->nome;
					$estado = tdClass::Criar("persistent",array("td_estado",$falida["estado"]))->contexto->nome;
...
"); // AR Detalhe - Quebra de linha
			...
			$sql = "SELECT * FROM td_relacaocredores WHERE processo = {$processo} and farein = {$farein} {$where_codigo} {$where_classificacao} and (origemcredor = 1 OR origemcredor = 10  OR origemcredor = NULL or origemcredor = '' or origemcredor is null) ORDER BY cep asc,classificacao asc, nome asc limit 49";
			...
			foreach ($result as $key => $value){
				...
				$natureza = tdClass::Criar("persistent",array("td_natureza",$value["natureza"]))->contexto->descricao;
				...
				$ARDetalhe->add(tdClass::Criar("persistent",array("td_estado",$credor->estado))->contexto->sigla); // AR Detalhe - Estado ( Sigla )
				...
				$ARDado->add("|");
				$ARDado->add(tdClass::Criar("persistent",array("td_estado",$credor->estado))->contexto->sigla);
				$ARDado->add("|");
