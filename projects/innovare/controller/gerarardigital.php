...
if (isset($_GET["op"])){
	if ($_GET["op"] == "download"){
		...
		try{
			...
			echo iconv("WINDOWS-1252//TRANSLIT", "ISO-8859-1//TRANSLIT", file_get_contents($arquivo));
			#echo iconv("WINDOWS-1252//TRANSLIT", "UTF-8", file_get_contents($arquivo));
...
$codigoservicocorreios 				= $configuracoesetiqueta->contexto->codigoservicocorreios;
$codigoservicoadicionalcorreios 	= $configuracoesetiqueta->contexto->codigoservicoadicionalcorreios;
...
foreach ($result as $key => $value){
	...
	$linha_sara .= $codigoservicocorreios;#fwrite($fpSara,"10065"); // Código do Serviço
	$linha_sara .= '10';#fwrite($fpSara,"10"); // Código do País
	$linha_sara .= $codigoservicoadicionalcorreios;#fwrite($fpSara,"25"); // Cod Serv Adic.
...
$style->add('
	...
	.group-buttons-download button
	{
		display:flex;
		justify-content: left;		
	}

	.group-buttons-download button i
	{
		margin: auto 10px;
	}

	#modal-download-ar .text-info
	{
		color:#111 !important;
	}
...
if (!$iserror){
	echo '
		<div class="btn-group group-buttons-download" role="group" aria-label="Baixar arquivos do AR Digital">
			<button type="button" class="btn btn-default" onclick=window.open(getURLProject("index.php?controller=gerarardigital&op=download&tipo=txt&arquivo='.$filenameDetalhe.'"),"_blank");>'.$icon_download.' <span>SGD</span></button>
			<button type="button" class="btn btn-default" onclick=window.open(getURLProject("index.php?controller=gerarardigital&op=download&tipo=txt&arquivo='.$filenameSara.'"),"_blank");>'.$icon_download.' <span>SARA</span></button>
			<button type="button" class="btn btn-default" onclick=window.open(getURLProject("index.php?controller=gerarardigital&op=download&tipo=txt&arquivo='.$filenameDado.'"),"_blank");>'.$icon_download.' <span>DADOS</span></button>
		</div>
	';
}