<?php
	...
	/*
		...
		if ($linha = $query->fetch()){
			...
			if ($tipo_farein == "RE"){ // Recupera&ccedil;&atilde;o
				...
				$sql = "SELECT razaosocial FROM td_recuperanda WHERE processo = $processo AND id={$farein}";
				$query = $conn->query($sql);
				if ($linha = $query->fetch()){
				...
				$sql = "SELECT razaosocial FROM td_falencia WHERE processo = $processo AND id={$farein}";
				$query = $conn->query($sql);
				if ($linha = $query->fetch()){
					$razaosocial =  $linha["razaosocial"];
	...
	*/	
		...
		$sql = "SELECT a.id,a.descricao FROM td_classificacao a WHERE exists(SELECT 1 FROM td_relacaocredores b WHERE b.classificacao = a.id AND b.processo = $processo AND b.farein = {$farein} AND b.origemcredor  = 4) ORDER BY a.ordem ASC ";
		...
		while ($linha = $query->fetch()){
			...
			$sql_credores = "SELECT nome,cpf,cnpj,valor,tipo,moeda FROM td_relacaocredores WHERE processo = $processo AND farein = $farein AND origemcredor = 4 AND classificacao = {$linha["id"]} ORDER BY nome ";
